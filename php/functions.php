...
class login_registration_class{
	...
	/**
	---------------------------------
	All functions for registration section
	---------------------------------
	**/
	public function fct_registration($name,$uname, $pass,$email, $bday,$gender,$contact,$address){
		global $conn;
		$fct = $conn->query("select username from registration where username='$uname' ");
		$count = $fct->num_rows;
		if($count == 0){
			$sql = "insert into registration(name,username,password,email,birthday,gender,contact,address) values('$name','$uname','$pass','$email','$bday','$gender','$contact','$address')";
			$result = $conn->query($sql);
			return true;
		}else{
			return false;
	...
	public function fct_login($uname, $pass){
		global $conn;
		$sql = "select username,name from registration where username='$uname' and password='$pass' ";
		$result = $conn->query($sql);
		$count = $result->num_rows;
	...
	*/
	...
	public function mgment_registration($name,$uname, $pass,$email, $bday,$gender,$contact,$address){
		global $conn;
		$fct = $conn->query("select username from mod_reg where username='$uname' ");
		$count = $fct->num_rows;
		if($count == 0){
			$sql = "insert into mod_reg(name,username,password,email,birthday,gender,contact,address) values('$name','$uname','$pass','$email','$bday','$gender','$contact','$address')";
			$result = $conn->query($sql);
			return true;
		}else{
			return false;
	...
	public function mgment_login($uname, $pass){
		global $conn;
		$sql = "select username,name from mod_reg where username='$uname' and password='$pass' ";
		$result = $conn->query($sql);
		$count = $result->num_rows;
		$fctinfo = $result->fetch_assoc();
		if($count == 1){
